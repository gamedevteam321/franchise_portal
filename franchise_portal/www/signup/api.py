    ...
    try:
        ...
        if email:
            ...
            if applications:
                ...
                if (step == 4 or step == 5) and 'generation_locations' in session_data['data'] and isinstance(session_data['data']['generation_locations'], list):
                    # Add debug logging
                    debug_file = os.path.join(frappe.get_site_path(), 'debug_save.txt')
                    with open(debug_file, 'a') as f:
        ...
        if data:
            ...
            for key, value in data.items():
                if key != 'generation_locations' and key in valid_fields:
                    updates[key] = value
        # Step 7 validation removed - step no longer exists
...
def finalize_application(session_data, token, current_step=5):
    try:
        ...
        with open(debug_file, 'a') as f:
            ...
            if 'generation_locations' in data:
                f.write(f"generation_locations: {data.generation_locations}\n")
                        ...
                        # Step 5, 6, 7 fields removed - steps no longer exist
...
def test_step5_fields():
    """Step 5 no longer exists - function removed"""
    return {"success": False, "message": "Step 5 no longer exists"}
